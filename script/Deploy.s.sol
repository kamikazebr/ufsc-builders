...
contract Deploy is Script {
    function run() external {
        ...
        // Collectible is the "one pinned JSON per token" example — the other half of
        ...
        //   DEPLOY_COLLECTIBLE=1 make deploy
        address collectible = address(0);
        if (vm.envOr("DEPLOY_COLLECTIBLE", false)) {
            collectible = address(new Collectible(msg.sender));
        }

        vm.stopBroadcast();

        console.log("token", address(token));
        if (collectible != address(0)) console.log("collectible", collectible);
        else console.log("collectible  skipped - DEPLOY_COLLECTIBLE=1 to include it");
        console.log("");
